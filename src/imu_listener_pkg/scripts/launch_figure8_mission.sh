...
MISSION_SCRIPT="$HOME/Ai_imu_ws/gestelt/src/imu_listener_pkg/scripts/figure8_launch.py"

# Verify paths
[ ! -f "$AI_CLIENT_PATH" ] && echo "[ERROR] AI client not found: $AI_CLIENT_PATH" && exit 1
[ ! -f "$MISSION_SCRIPT" ] && echo "[ERROR] Mission script not found: $MISSION_SCRIPT" && exit 1
...
# Pane 4: Figure-8 Mission Launcher (waits for everything to be ready)
tmux split-window -v -t "$SESSION":0.3
tmux select-pane -t "$SESSION":0.4 -T "Figure-8 Mission"
tmux send-keys -t "$SESSION":0.4 "echo 'Waiting 25s for system to initialize...' && sleep 25 && echo 'Launching mission...' && python3 $MISSION_SCRIPT" C-m

# Note: Rosbag recording is automatic - starts/stops with AI client for perfect time alignment
