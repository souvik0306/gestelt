...
def main():
    ...
    # Request zero velocity *and* zero acceleration at every corner so the
    # planner is forced to stop and leave each vertex without curvature. This
    # prevents the minimum-snap optimizer from rounding the turns.
    velocities = [create_vel(0.0, 0.0, 0.0) for _ in corners]
    accelerations = [create_accel(0.0, 0.0, 0.0) for _ in corners]
