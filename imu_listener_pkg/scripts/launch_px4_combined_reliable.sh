...
tmux attach -t "gz_sim_single_uav" \; \
  ...
  send-keys -t 2 "sleep 2 && source $HOME/Ai_imu_ws_noise/devel/setup.bash && export ROS_PACKAGE_PATH=$HOME/Ai_imu_ws_noise/src:\$ROS_PACKAGE_PATH && roslaunch trajectory_server trajectory_server_node.launch rviz_config:=gz_sim" C-m \; \
  send-keys -t 3 "sleep 15 && source $HOME/Ai_imu_ws_noise/devel/setup.bash && export ROS_PACKAGE_PATH=$HOME/Ai_imu_ws_noise/src:\$ROS_PACKAGE_PATH && roslaunch trajectory_planner trajectory_planner_node.launch" C-m \; \
  send-keys -t 4 "sleep 50 & source $HOME/Ai_imu_ws_noise/devel/setup.bash && export ROS_PACKAGE_PATH=$HOME/Ai_imu_ws_noise/src:\$ROS_PACKAGE_PATH && roslaunch gestelt_bringup demo_trajectory_mission.launch" C-m